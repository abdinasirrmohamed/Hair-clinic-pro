...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        $stmt = $conn->prepare('UPDATE doctors SET user_id = ?, full_name = ?, specialization = ?, phone = ?, email = ?, license_number = ?, status = ? WHERE id = ?');
        $stmt->bind_param('issssssi', $user_id, $full_name, $specialization, $phone, $email, $license_number, $status, $id);
        try {
            $stmt->execute();
