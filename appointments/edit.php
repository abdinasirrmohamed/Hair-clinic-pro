...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $stmt = $conn->prepare('UPDATE appointments SET patient_id = ?, appointment_date = ?, appointment_time = ?, reason = ?, status = ?, notes = ? WHERE id = ?');
    $patient_id = (int) $_POST['patient_id'];
    $stmt->bind_param('isssssi', $patient_id, $_POST['appointment_date'], $_POST['appointment_time'], $_POST['reason'], $_POST['status'], $_POST['notes'], $id);
    $stmt->execute();
...
<form class="form-panel" method="post">
    <div class="row g-3">
        ...
        <div class="col-md-4"><label class="form-label">Status</label><select class="form-select" name="status"><?php foreach (['Pending','Completed','Cancelled'] as $s): ?><option <?= $appt['status'] === $s ? 'selected' : '' ?>><?= $s ?></option><?php endforeach; ?></select></div>
