...
export const modules = {
  ...
  treatments: {
    ...
    labels: { 'patient.full_name': 'Patient', treatment_name: 'Treatment Plan', treatment_date: 'Date', treatment_stage: 'Stage', progress: 'Progress', cost: 'Cost' },
    createDefaults: { treatment_stage: 'Pre-Treatment Evaluation', progress: 'Started', cost: 0 },
    fields: [
      field('patient_id', 'Patient', 'lookup', { lookup: 'patients' }),
      field('treatment_name', 'Treatment Plan', 'text', { maxLength: 150 }),
      field('treatment_date', 'Treatment Date', 'date'),
      field('treatment_stage', 'Stage', 'select', { options: ['Pre-Treatment Evaluation', 'Surgery', 'Post-Treatment Review'] }),
      field('progress', 'Progress', 'select', { options: ['Started', 'In Progress', 'Completed'] }),
      field('cost', 'Cost', 'number'),
      ...['planned', 'extracted', 'implanted'].map((stage) => field(`grafts_${stage}`, `Grafts ${stage}`, 'number', { step: 1, max: 2147483647 })),
      field('donor_area_status', 'Donor Area Status', 'text', { maxLength: 255 }),
      field('recipient_area_status', 'Recipient Area Status', 'text', { maxLength: 255 }),
      field('notes', 'Plan / Notes', 'textarea'),
    ],
  },
  followups: {
...
const requiredFields = {
  ...
  treatments: ['patient_id', 'treatment_name', 'treatment_date', 'treatment_stage', 'progress', 'cost'],
