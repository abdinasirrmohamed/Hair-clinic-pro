...
$direct_sales = count_table($conn, 'SELECT COUNT(*) FROM pharmacy_sales WHERE prescription_id IS NULL');
$prescription_sales = count_table($conn, 'SELECT COUNT(*) FROM pharmacy_sales WHERE prescription_id IS NOT NULL');
$returned_sales = count_table($conn, "SELECT COUNT(*) FROM pharmacy_sales WHERE status = 'Returned'");
...
<div class="patient-metrics">
    ...
    <article class="patient-metric"><div><p>Direct Sales</p><strong><?= number_format($direct_sales) ?></strong></div><span class="metric-icon mint"><i class="bi bi-cart-check"></i></span></article>
    ...
    <article class="patient-metric"><div><p>Returned Sales</p><strong><?= number_format($returned_sales) ?></strong></div><span class="metric-icon pale-red"><i class="bi bi-arrow-counterclockwise"></i></span></article>
...
<section class="patient-management-card mb-4">
    <div class="patient-tabs"><div class="tab-links"><a href="medicines.php">Medicines</a><a href="sale.php">New Sale</a><a href="prescriptions.php">Prescriptions</a><a href="sales_history.php">Sales History</a><a class="active" href="reports.php">Reports</a></div></div>
    <div class="patient-list-grid patient-list-head" style="grid-template-columns: 1fr 1.15fr .9fr .65fr .75fr .9fr .8fr;">
        <span>Sale Number</span><span>Customer</span><span>Type</span><span>Items</span><span>Total</span><span>Method</span><span>Status</span>
    </div>
    <?php foreach ($sales as $sale): ?>
        <div class="patient-list-grid patient-list-row" style="grid-template-columns: 1fr 1.15fr .9fr .65fr .75fr .9fr .8fr;">
            ...
            <span><?= $sale['prescription_id'] ? 'Prescription' : 'Direct Sale' ?></span>
